...
std::tuple<bool, double> mean_test(
    ...
    H1Comparison comparison)
{
    auto t_cal = mean_statistic(sample_mean, sample_std_dev, sample_size, mean);
    auto degrees_of_freedom = (double)sample_size - 1;
    auto p_value = mean_p_value(t_cal, degrees_of_freedom, comparison);

    auto t_crit = mean_critical_value(confidence_level, degrees_of_freedom, comparison);
